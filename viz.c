...
static void show_one(Rule *r, int size, int steps, const char *init, Rng *rng) {
    ...
    if (strcmp(init, "patch") == 0) {
        int start = size / 2 - 10;
        for (int i = 0; i < 20; i++) cur[((start + i) % size + size) % size] = (uint8_t)rng_int(rng, r->K);
    } else {
        for (int i = 0; i < size; i++) cur[i] = (uint8_t)rng_int(rng, r->K);
    }
    for (int t = 0; t < steps; t++) {
        for (int i = 0; i < size; i++) putchar(GLYPHS[cur[i]]);
        putchar('\n');
...
void run_viz(int argc, char **argv) {
    ...
    Rule *r = ca_new(K, N, 1);
    if (!r) { fprintf(stderr, "viz: invalid params (need 2<=K<=8, odd N, dim=1)\n"); exit(1); }
    ...
    if (sweep) {
        ...
        for (double L = 0.05; L <= lmax + 1e-9; L += 0.05) {
            ca_walk_to(r, L, &rng);
            printf("\n=== lambda ~ %.2f (realized %.3f) ===\n", L, ca_realized_lambda(r));
            show_one(r, size, steps, init, &rng);
        }
    } else {
        ca_gen_random(r, lambda, &rng);
        printf("# K=%d N=%d size=%d  lambda target %.3f realized %.3f  init=%s\n",

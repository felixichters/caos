/* Cellular automaton engine: lambda-parameterized transition tables (Langton
 * 1990). */
#ifndef CA_H
#define CA_H
#include "rng.h"
...
/* random-table method: each canonical class -> Sq with prob (1-lambda), else
 * random non-Sq */
void ca_gen_random(Rule *r, double lambda, Rng *rng);

/* table-walk-through method: start quiescent (lambda~0), perturb the same table
 * toward target */

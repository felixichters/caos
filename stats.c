...
static void step_grid(const Rule *r, const uint8_t *cur, uint8_t *next, int w, int h) {
    if (r->dim == 1) ca_step_1d(r, cur, next, w);
    else             ca_step_2d(r, cur, next, w, h);
}
...
static double entropy_hist(const long *c, int K, long total) {
    ...
    double H = 0.0;
    for (int i = 0; i < K; i++) {
        if (c[i]) { double p = (double)c[i] / total; H -= p * log2(p); }
    }
    return H;
}
...
static long transient_len(Rule *r, int w, int h, long tcap, Rng *rng) {
    ...
    for (int i = 0; i < n; i++) cur[i] = (uint8_t)rng_int(rng, r->K);

    size_t cap = 1;
    while (cap < (size_t)(tcap * 2 + 8)) cap <<= 1;
    ...
    for (long t = 0; t < tcap; t++) {
        ...
        size_t p = hsh & mask;
        int found = 0;
        while (set[p]) { if (set[p] == hsh) { found = 1; break; } p = (p + 1) & mask; }
        if (found) { result = t; break; }
        set[p] = hsh;
        step_grid(r, cur, next, w, h);
        tmp = cur; cur = next; next = tmp;
    }
    free(set); free(cur); free(next);
    return result;
}

Measures ca_measure(Rule *r, int w, int h, int settle, int sample, long tcap, Rng *rng) {
    int n = w * h, K = r->K;
    uint8_t *cur = malloc(n), *next = malloc(n), *tmp;
    for (int i = 0; i < n; i++) cur[i] = (uint8_t)rng_int(rng, r->K);
    ...
    long js[CA_MAXK * CA_MAXK] = {0};   /* spatial joint: cur vs right neighbor */
    ...
    for (int t = 0; t < sample; t++) {
        ...
        for (int i = 0; i < n; i++) {
            single[cur[i]]++; total++;
            jt[cur[i] * K + next[i]]++; totalt++;
        }
        if (r->dim == 1) {
            for (int i = 0; i < n; i++) {
                int jx = (i + 1) % w;
                js[cur[i] * K + cur[jx]]++; totals++;
            }
        } else {
            for (int y = 0; y < h; y++)
                for (int x = 0; x < w; x++) {
                    int i = y * w + x, jx = y * w + (x + 1) % w;
                    js[cur[i] * K + cur[jx]]++; totals++;
                }
        }
        tmp = cur; cur = next; next = tmp;

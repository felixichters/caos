/* Lambda sweep: many random tables per lambda, emit measures as TSV (Langton 1990, section 5). */
#include "ca.h"
#include "stats.h"
#include "args.h"
#include <stdio.h>
...
void run_sweep(int argc, char **argv) {
    ...
    if (lmax < 0) lmax = 1.0 - 1.0 / K;

    Rule *r = ca_new(K, N, dim);
    if (!r) {
        fprintf(stderr, "sweep: invalid params (need 2<=K<=8; dim=1 odd N; dim=2 N=5)\n");
        exit(1);
    }
    ...
    printf("# caos sweep dim=%d K=%d N=%d size=%d runs=%d settle=%d sample=%d tcap=%ld\n",
    ...
    for (double L = lmin; L <= lmax + 1e-9; L += lstep) {
        ...
        for (int run = 0; run < runs; run++) {
            ...
            Measures m = ca_measure(r, w, h, settle, sample, tcap, &rng);
            if (agg) {
                sH += m.H; sHn += m.H_norm; smt += m.mi_t; sms += m.mi_s; stt += m.transient; cnt++;
            } else {
                printf("%.4f\t%.5f\t%.5f\t%.5f\t%.5f\t%ld\n",
                       rl, m.H, m.H_norm, m.mi_t, m.mi_s, m.transient);
            }
        }
        if (agg && cnt) {
            printf("%.4f\t%.5f\t%.5f\t%.5f\t%.5f\t%.2f\n",
                   L, sH / cnt, sHn / cnt, smt / cnt, sms / cnt, stt / cnt);
        }
        fflush(stdout);
    }
    ca_free(r);
